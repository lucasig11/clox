...
static void emit_loop(int loop_start) {
  ...
  // +2 for the instruction operands
  int offset = current_chunk()->length - loop_start + 2;
...
static void end_scope() {
  ...
  while (current->local_count > 0 &&
         current->locals[current->local_count - 1].depth >
             current->scope_depth) {
    // Hoist captured variables to the heap
...
static void dot(bool can_assign) {
  ...
  // Load idientifier lexeme string into constant pool, so we can use it at
  // runtime.
...
static void for_statement() {
  ...
  if (!match(TOKEN_SEMICOLON)) {
    ...
    // Pop the condition out of the stack
    emit_byte(OP_POP);
  }
  if (!match(TOKEN_RIGHT_PAREN)) {
    int body_jump = emit_jump(OP_JUMP);
    int increment_start = current_chunk()->length;
    expression();
